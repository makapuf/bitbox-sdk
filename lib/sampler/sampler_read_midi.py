...
def read_midi(midifile):
    ...
    for i in range(ntracks):
        ...
        if set_tempo:
            tempo = set_tempo
        if not events:
            continue  # skip empty track - First Type 1 midi often
        print("Track track_%s={" % track_name)
        for tick, evttype, p1, p2, comment in events:
            print(
                "    {.tick=%4d, .type=%s, .p1=0x%02x, .p2=0x%02x}, // %s"
                % (tick, evttype, p1, p2, comment)
            )
        tot_evts += len(events)
        print("}")
...
def read_midi_simple(midifile):
    ...
    for i in range(ntracks):
        ...
        for tick, evttype, p1, p2, comment in events:
            ...
            if evttype == "midi_note_on":
                print(
                    "    {.tick=%4d, .note=0x%02x, .vel=0x%02x}, // %s"
                    % (frame, p1, p2, comment)
                )
            elif evttype == "midi_note_off":
                print(
                    "    {.tick=%4d, .note=0x%02x, .vel=0}, // note off " % (frame, p1)
                )
            else:
                pass  # ignore
    ...
    print(
        "const int track_%s_len = %d; // %d kb"
        % (track_name.lower(), tot_evts, tot_evts * 4 // 1024)
    )
    return format, ntracks, resolution, tot_evts
...
def parse_file_header(midifile):
    ...
    assert header == b"MThd", f"Bad header in MIDI file, got {header}."
...
def parse_track(midifile):
    ...
    assert midifile.read(4) == b"MTrk", "Bad track header in MIDI file on byte %d." % (
        midifile.tell() - 4
    )
    ...
    trackfile = io.BytesIO(trackdata)
    ...
    while True:
        ...
        if status == 0xFF:  # meta event
            ...
            if meta_type == 47:  # end of track
                ...
                track_name = meta_data.decode("utf8")
                print("// Track name :", track_name)
            else:
                print(
                    "// (meta) %s :"
                    % META_TYPES.get(meta_type, "??? (%d)" % meta_type),
                    repr(meta_data),
                )
